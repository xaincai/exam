				...
				<div class="row">
					...
					<div class="col-xs-7" id="login-info">
						<c:choose>
							...
							<c:otherwise>
								<a class="btn btn-success" href="user-register">用户注册</a>
								<a class="btn btn-success" href="user-login-page">用户登录</a>
								<a class="btn btn-primary" href="adLogin">管理员登录</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
										     ...
										     <div class="modal-body">
										     	<ul>
										     		<c:forEach items="${wrongKnowledgeMap }" var="item" >
										     			<li><span class="point-name"> ${item.key }</span> [共<span class="question-number-3">
										     			<c:forEach items="${item.value }" var="v">${v.value }</c:forEach> 
										     			</span>题]
